        ...
        <ion-toolbar>
            ...
            <div class="flex flex-col justify-center items-center mt-2">
                <div class="text-center">
                    <ion-icon :icon="easelOutline" size="large" color="primary"></ion-icon>
                </div>

                <div class="text-center">
                    <ion-card-title class="text-2xl">Seminars</ion-card-title>
...
import {ellipsisVertical,easelOutline,trash,add} from 'ionicons/icons';
...
export default defineComponent({
    ...
    setup(){
        ...
        const state = reactive({
            tasksSeminars: computed(() => {
                return store.getters.tasksByCategory('Seminar');
            }),
            today: computed(() => {
                return store.getters.today(state.tasksSeminars);
            }),
            late: computed(() => {
                return store.getters.late(state.tasksSeminars);
            }),
            later: computed(() => {
                return store.getters.later(state.tasksSeminars);
            }),
            done: computed(() => {
                return store.getters.done(state.tasksSeminars);
            })
        })
        function getTasksSeminars() {
        ...
        return{
            isOpenNewTask,store,state,getTasksSeminars,doneTask,notDoneTask,deleteTask,
            ellipsisVertical,easelOutline,trash,add
        }
    }
})
</script>
